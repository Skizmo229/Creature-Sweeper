# Working notes as of 24 September 2026 (superseded)

This is the CLAUDE.md that Milestone 3 phase 1 replaced, kept verbatim as the source the files in
`docs/` were condensed from. It is not maintained. Where it and `docs/` disagree, `docs/` is
current; where `docs/` is silent on a detail, this is where the detail still is, until the comment
pass (phase 4) or a decision record moves it.
...
(689 boards in all). 409 tests. Playable prototype with canvas board, HUD, marks,
