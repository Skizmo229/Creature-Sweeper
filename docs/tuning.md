...
| --- | --- |
| `npm run sim [-- seeds]` | Clears every one of the 722 boards with the omniscient tier-order player. Reports the opening and HP lost; exits non-zero if any board cannot be cleared at full HP. The regression gate for `ladders.py`. |
...
- The counted unlock schedule opens one ladder per menu category every five boards from 15, BLIND
  last at 55, in a hand-set order that does not follow difficulty (decision 0036). `test/unlocks.test.ts` walks it and fails if any gate is unreachable on tuned
  boards alone.
...
7. **Smaller:** BLIND's unlock timing (55 boards) is a guess; pinch-zoom has only met
