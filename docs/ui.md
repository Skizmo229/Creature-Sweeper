...
- The first clear of a board holds its card back for three seconds, by request, so the clear effect
  ...
  panned out from under the effect and no hidden button can be pressed; Escape still leaves. A
  first clear is one the save had never recorded. A replay, a loss and a clear with the effect off
  (nothing to watch) show the card at once. A Full Run never holds one: its boards were all
  cleared before it opened, and between boards its clock is still running.
