...
npm test               # 503 tests, including the invariants
npm run sim:golden:check   # re-run fourteen fixed-seed simulator runs and diff against test/golden/
npm run sim            # clear every one of the 737 boards headlessly (-- 200 for more seeds)
...
**Game types.** 27 ladders of ten tuned boards, each with a scaling continuation past board 10
(737 boards in all) and a Full Run (all ten on one HP pool).
