...
npm test               # 514 tests, including the invariants
npm run sim:golden:check   # re-run fourteen fixed-seed simulator runs and diff against test/golden/
npm run sim            # clear every one of the 814 boards headlessly (-- 200 for more seeds)
...
**Game types.** 31 ladders of ten tuned boards, each with a scaling continuation past board 10
(814 boards in all) and a Full Run (all ten on one HP pool).
