...
| --- | --- |
| `npm run sim [-- seeds]` | Clears every one of the 791 boards with the omniscient tier-order player. Reports the opening and HP lost; exits non-zero if any board cannot be cleared at full HP. The regression gate for `ladders.py`. |
