...
│  │  ├─ congo.ts            the congo placement: packs strung into lines, led by the tier 6
...
`npm run sim` is the tuning instrument: it plays all 689 boards with an omniscient tier-order
...
at its generator's floor, HUGE x EXTREME runs to 34 since its retune left it room, and the gentler
...
the same invariants, and `npm run sim` clears all 689 of them at full HP.
